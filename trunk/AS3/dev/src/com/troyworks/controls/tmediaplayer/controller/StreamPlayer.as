package com.troyworks.mediaplayer.controller { 
	...
	{
		public static var NC_CONNECT_SUCCESS_EVT : AEvent = AEvent.getNext("NetConnection.Connect.Success_SIG");
		public static var NC_CONNECT_FAILED_EVT : AEvent =  AEvent.getNext( "NetConnection.Connect.Failed_SIG");
		public static var NS_PLAY_STREAMNOTFOUND_EVT : AEvent = AEvent.getNext("NetStream.Play.StreamNotFound_SIG");
		public static var NS_BUFFER_FULL_EVT : AEvent =  AEvent.getNext("NetStream.Buffer.Full_SIG");
		public static var NS_BUFFER_EMPTY_EVT : AEvent =  AEvent.getNext("NetStream.Buffer.Empty_SIG");
		public static var NS_BUFFER_FLUSH_EVT : AEvent =  AEvent.getNext("NetStream.Buffer.Flush_SIG");
		public static var NS_PAUSE_NOTIFY_EVT : AEvent =  AEvent.getNext("NetStream.Pause.Notify_SIG");
		public static var NS_PLAY_START_EVT : AEvent =  AEvent.getNext("NetStream.Play.Start_SIG");
		public static var NS_PLAY_RESET_EVT : AEvent =  AEvent.getNext("NetStream.Play.Reset_SIG");
		public static var NS_PLAY_STOP_EVT : AEvent = AEvent.getNext("NetStream.Play.Stop_SIG");
		public static var NC_CONNECT_REJECTED_EVT : AEvent =  AEvent.getNext("NetConnection.Connect.Rejected_SIG");
		public static var NC_CONNECT_CLOSED_EVT : AEvent = AEvent.getNext("NetConnection.Connect.Closed_SIG");
		public static var NS_SEEK_NOTIFY_EVT : AEvent =  AEvent.getNext("NetStream.Seek.Notify_SIG");
		...
		*/
		...
		public function connectToMediaStream () : void
		{
			...
			//	this.nsPauseStream ();
			//this.nsResumePlay();
			//this.Q_TRAN (this.s1a_preloading);
			//}
		}
		...
		*/
		...
		public function onNCStatus (oInfo : Object) : void
		{
			...
			switch (oInfo.code)
			{
				...
				this.Q_dispatch (NC_CONNECT_SUCCESS_EVT);
				break;
				case "NetConnection.Connect.Failed" :
				this.connect_mc.gotoAndStop ("red");
				this.Q_dispatch (NC_CONNECT_FAILED_EVT);
				break;
				case "NetConnection.Connect.Closed" :
				this.connect_mc.gotoAndStop ("red");
				this.Q_dispatch (NC_CONNECT_CLOSED_EVT);
				break;
				case "NetConnection.Connect.Rejected" :
				this.connect_mc.gotoAndStop ("red");
				this.Q_dispatch (NC_CONNECT_REJECTED_EVT);
				break;
			}
		}
		...
		function onNSStatus (oInfo : Object) : void
		{
			...
			switch (oInfo.code)
			{
				...
				this.Q_dispatch (NS_PLAY_STREAMNOTFOUND_EVT);
				break;
				case "NetStream.Buffer.Full" :
				{
					this.Q_dispatch (NS_BUFFER_FULL_EVT);
				}
				break;
				case "NetStream.Buffer.Empty" :
				{
					...
					this.Q_dispatch (NS_BUFFER_EMPTY_EVT);
				}
				break;
				case "NetStream.Buffer.Flush" :
				this.stream_mc.gotoAndStop ("gray");
				//Data has finished streaming, and the remaining buffer will be emptied.
				this.Q_dispatch (NS_BUFFER_FLUSH_EVT);
				break;
				case "NetStream.Pause.Notify" :
				//"The seek operation is complete."
				this.Q_dispatch (NS_PAUSE_NOTIFY_EVT);
				break;
				case "NetStream.Play.Start" :
				{
					//Playback/BUffering has started.
					this.startBandwidthTestFromBuffer ();
					this.Q_dispatch (NS_PLAY_START_EVT);
				}
				break;
				case "NetStream.Play.Reset" :
				this.Q_dispatch (NS_PLAY_RESET_EVT);
				break;
				case "NetStream.Play.Stop" :
				//Playback has stopped.
				this.Q_dispatch (NS_PLAY_STOP_EVT);
				break;
				case "NetStream.Seek.Notify" :
				this.Q_dispatch (NS_SEEK_NOTIFY_EVT);
				break;
		...
		/*.................................................................*/
		function s0_awaitingUILoad (e : AEvent) : Function
		{
		...
		function s1_active (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				case NS_PAUSE_NOTIFY_EVT :
				{
					if (this.ns.time == 0)
					{
						this.Q_TRAN (s10_pausedAtBeginning);
					} else
					{
						this.Q_TRAN (s13_pausedInMiddle);
					}
				}
				return null;
			}
		...
		function s1a_preloading (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				case PULSE_EVT :
				{
					...
					if (this.minimumBufferLengthToNotRebuffer > 0 && this.minimumBufferLengthToNotRebuffer < this.ns.bufferLength)
					{
						...
						if(Track.IDz > 1){
							this.Q_TRAN (this.s1b_loadingButReadyToPlay);
						}else{
		...
		function s1b_loadingButReadyToPlay (e : AEvent) : Function
		{
		...
		function s11_playing (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				trace("NS playstop while playing...stream ending?" + this.ns.time + " " + this.clipDuration + " " + this.ns.bufferLength + " " + (this.ns.time > (this.clipDuration - this.ns.bufferLength)));
				if (this.ns.time + .5 >= (this.clipDuration - this.ns.bufferLength))
				{
					this.Q_TRAN (this.s11b_playingEndOfStream);
				}
				return null;
				case NS_BUFFER_FLUSH_EVT:
				//if comingfrom buffering well hit this
				this.Q_TRAN (this.s11b_playingEndOfStream);
				return null;
				case NS_PLAY_START_EVT :
				return null;
				case NS_BUFFER_EMPTY_EVT:
				this.Q_TRAN(this.s13_buffering);
				return null;
			}
			return this.s1_active;
		}
		function s11b_playingEndOfStream (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				if (this.ns.time > (this.clipDuration - this.ns.bufferTime))
				{
					...
					this.Q_TRAN (this.s13_pausedAtEnd);
				} else
				{
					trace ("!!!!!!!!!!!!!!!!!!!!Buffereing!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!");
				}
		...
		function s13_pausedAtEnd (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				if (this.autoRewind)
				{
					this.Q_dispatch (GOTOANDSTOP_EVT);
				}
				return null;
				case EXIT_EVT :
				this.stopPulse();
		...
		function s13_seeking (e : AEvent) : Function
		{
		...
		function s13_buffering (e : AEvent) : Function
		{
			...
			switch (e)
			{
				...
				return null;
				case NS_PLAY_START_EVT:
				//started filling buffer (by chance)
				return null;
				case NS_BUFFER_FULL_EVT :
				...
				{
					//the stream is finished buffering (used to think it was restarting) restart it.
					trace ("*8888888888888888888888888888888Restarting Stream*******************************************************");
					//this.nsResumePlay ();
					this.Q_TRAN (this.s11_playing);
				}
				return null;
		...
		function s14_NCerror (e : AEvent) : Function
		{
		...
		function s15_NSerror (e : AEvent) : Function
		{
