...
package com.troyworks.core.tweeny {
	...
	public class TnyF extends Sprite{
		...
		public function TnyF(targetD:DisplayObject = null) {
		...
		*/
		...
		/*	if(isNaN(dur)){
			...
			//	trace("duration " + dur + " at  " + fps + " fps");
				d = (dur* fps) - 1; // how many frames it will take to get there, keeping in mind that the start frame and end frame take up space.
				d = (d< 2)?2:d; // minimum 2 frame duration
				ts = 1; // points crossed - 1 = steps crossed. ts is a percentage to move.
				trace("ts " + ts + " =  1/" +(d-1));
				
				t = -dl; // if we have a delay, setup the delay as a percentage of the time (as we increment in percent)
				if(!lockStep){
					t += ts * Math.random();
				}
			}
...
//trace(" t " + t + " " + dd);
			if(t >= (d - (ts * .5))){
				// finished
		trace(trg.name +" isFinishing "+ t + "% " + d  );
	...
	//	trace(trg.name +" active "+ t + "% " + d + " frame: " + fc + " " + + (getTimer() - st));
			...
			//	trace("finishing ColorTransform----------");
					...
					trg.visible = trg.transform.colorTransform.alphaMultiplier > 0;
