...
class TestSOSLogger {
	...
	static function main(mc : MovieClip) : void {
		...
		_root.testSOSLogger.runTests();

		mc.createTextField("tf2",1,40,40,800,600);

		// write some text into it
		mc.tf2.text = " From" + mc._name + " _url:" + mc._url ;
...
//		testSOSLoggerMessageColoring();
	...
	public function testSOSLogger_BasicData() : void{
		...
		var _ary : Array = ["A","B","C","D"];
		_root.logger.log("Array");
		_root.logger.log(_ary);
		_root.logger.logBreak();
		_root.logger.log(util.Trace.me(_ary, "Array2", true));
		_root.logger.logBreak();
		_root.logger.showMessage("Message to SOS");
		_root.logger.clearLog();
		testMessageColoring();
    ...
    * something: undefined
	...
	public function testSOS_CommandIdentify() : void{
		...
		csock.send("<commands>"
			+ "<appName>Some Name</appName>"
			+ "<appColor>16768477</appColor>"
			+ "<identify/>"
			+ "<showMessage>Test</showMessage>"
			+ "</commands>\n"
		);
	}
	...
	public function testSOS_foldedMessage() : void{
		...
		msock.send("<showFoldMessage key='MyFirstKey'>"
		+ "<title>foldMessage1</title>"
		+ "<message>This is a multiline message:\nShow some Object...\n\ta: 1,\n\tb: 2</message>"
		+ "</showFoldMessage>"
		);

		msock.send("<showFoldMessage>"
		+ "<title>foldMessage2 - without key</title>"
		+ "<message>This is a multiline message:\nShow some Object...\n\ta: 1,\n\tb: 2</message>"
		+ "</showFoldMessage>"
		);
	...
	public function testSOS_foldedMessage2() : void{
		...
		csock.send("<showFoldMessage key='MyFirstKey'>"
		+ "<title>foldMessage2a</title>"
		+ "<message>This is a multiline message:\nShow some Object...\n\ta: 1,\n\tb: 2</message>"
		+"<showFoldMessage>"
		+ "<title>foldMessage2b - without key</title>"
		+ "<message>This is a multiline message:\nShow some Object...\n\ta: 1,\n\tb: 2</message>"
		+ "</showFoldMessage>"
		+ "</showFoldMessage>"
		);
	...
	public function testLoginDialogBox() : void{
		...
		msock.send("<commands>"+
		 "<showDialog>"+
			...
			"<component>"+
			'<input name="Host" param="$host" value="fc.powerflasher.de" />'+
			'<input name="Port" param="$port" value="1111" />'+
			'<input name="Username" param="$username" value="admin" />'+
			'<inputPassword name="Password" param="$password" value="password" />'+
			"</component>"+
			"<execute>"+
				'<sendApp name="FCAdmin"><![CDATA[<selectInstance inst="$inst"/>]]></sendApp>'+
			"</execute>"+
		  "</showDialog>"+
	     "</commands>\n");
	}
	...
	public function testDialogWithComboBox() : void{
		...
		msock.send("<commands>"+
		 "<showDialog>"+
			"<title>FCAdmin - Select Instance</title>"+
			"<description>Please select a FlashCom Instance</description>"+
			"<component>"+
				'<comboBox name="Instance" param="$inst">'+
					'<item name="Instance Name 1"/>'+
					'<item name="Instance Name 2"/>'+
					'<item name="Instance Name 3"/>'+
				"</comboBox>"+
			"</component>"+
			"<execute>"+
				'<sendApp name="FCAdmin"><![CDATA[<selectInstance inst="$inst"/>]]></sendApp>'+
	...
	public function testErrorDialog() : void{
		...
		csock.send("<commands>"+
			...
			"<title>MyErrorTitle</title>"+
			"<message>MyErrorMessage</message>"+
			"</showError>" +
 		     "</commands>\n");
	}
