package com.troyworks.controls.tmediaplayer.ui {
	...
	public class PlayerNav extends BaseComponent {
		...
		public function onPlayerStateChanged(_ary : ArrayX) : void {
			...
			var _ary2 : ArrayX = _ary;
			// ArrayX(_ary[0]);
			//		_ary2.appendArray(_ary[1]);
			trace("HIGHLIGHTP 1111111111111111111111  PlayerNav.onPlayerStateChanged  " + _ary.length + "  11111111111111111111111 \r" + _ary.join("\r"));
			//util.Trace.me(_ary, "_ary", true));
...
//			this.onFunctionEnter ("s1_viewCreated-", e, []);
			switch (e.sig)
			{
				...
				case SIG_STATE_CHANGED:
					{
				//	trace(util.Trace.me(Object(e).args, " HANDLING EVENT-------------------", true));
	
	//TODO				onPlayerStateChanged(Object(e).args[0].stackOpts);
					return null;
				}
				default:
