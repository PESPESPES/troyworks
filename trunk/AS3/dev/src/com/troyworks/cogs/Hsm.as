package com.troyworks.cogs{
	...
	public class Hsm extends EventDispatcher {
		...
		public var trace:Function = Hsm.getDefaultTrace();
		...
		protected var _hsmName : String = "hsmE";


		protected var _hsmID:uint;
		//////////////// HSM STATE //////////////////////
		protected var _hasInited:Boolean = false;
		/* function pointer to initial pseudo or history state*/
		...
		/* for asynchronous and queed transitions */
		 ...
		 * 
		 * ********************************************************************/
		public function Hsm(initStateNameAct : String =null, hsmName : String =null, aInit : Boolean = true):void {
			super();
			trace("HIGHLIGHTB new HSM(" + hsmName+")");
	...
	//		trace("attemping to get initStateName " + initStateName);
			...
			REQUIRE(hsmName != null,   " hsm %1 requires an a name passed into constructor", hsmID);
			REQUIRE(initState != null,  " %1 hsm %2 requires an initial state passed into constructor", hsmName, hsmID);
			REQUIRE(initState != s_root, "% 1 hsm %2  requires an initial state (not Root_st) passed into constructor", hsmName, hsmID);
...
//			
		...
		//////////////////// ACCESSORS ////////////////////////////
		public function get hsmID():uint {
			return _hsmID;
		}
		public function get hsmName():String {
			return _hsmName;
		}
		public function get hsm_hasInited():Boolean {
	...
	//use namespace  COG;
			...
			if (event is CogEvent) {
							trace(hsmName + "." + getStateName(_myCurState) + ":Hsm.dispatchEvent Cog " + event.type);
				if(myCurState != null){
					s = myCurState;
					while (true) {
						mySource = s;
		...
		public function requestTranNoInit(targetState:Function):void{
			requestTran(targetState, false);
		}
		public function requestTran(targetState:Function, doInitDiscovery:Boolean = true):void{
			trace("requestTran");
				//use namespace  COG;
		...
		//	REQUIRE(targetState is Function, "***ERROR*** %1.QTRAN, passed invalid state function:  %2", toStringShort (),(targetState is Function));
			...
			var q:QueuedTransitionRequest = new QueuedTransitionRequest(targetState, doInitDiscovery);
			evtque.push(q);
		...
		protected function onTranTimer(event:TimerEvent):void{
			...
			var q:QueuedTransitionRequest = QueuedTransitionRequest(evtque.shift());
			setCurrentState(q.trg, q.doInitDiscovery);
		 ...
		 * 
		 * **********************************************/
		 protected function tran(targetState:Function, doInitDiscovery:Boolean = true ):void {
			 	//use namespace  COG;
			REQUIRE(targetState != s_root, "cannot transition to root state!");
			setCurrentState(targetState, doInitDiscovery);	
		 }
		/******************************************
		...
		*/
		public function setCurrentState(targetState:Function, doInitDiscovery:Boolean = true):void {
				//use namespace  COG;
			//REQUIRE(targetState != null, "***ERROR*** %1.tran, passed null state function" ,  toStringShort ());
			REQUIRE(targetState != s_root, "cannot transition to root state!");
		//	REQUIRE(targetState is Function, "***ERROR*** %1.QTRAN, passed invalid state function:  %2", toStringShort (),(targetState is Function));
		...
		//	trace("\r\r");
			...
			*/
			...
			transLock = true;
			trace(hsmName + " TRANSITIONING===================");
			var i:uint;
			var finalState:Function;
		
			var msg:String;
			var handled:Function = null;
		...
		*/
		protected function onInternalStateChanged():void {
			...
			if ( hsmIsActive && hasEventListener(CogExternalEvent.CHANGED )) {
				...
				dispatchEvent(cogE);
					...
					trace("  no notifications " + hsmIsActive);
			}
		}
		...
		override public function toString():String {
			...
			a.push("Hsm." + hsmName + hsmID + "." + myCurStateName);
			return a.join("\r");
		}
		public function toStringShort(stateName : String= null):String {
			var res : String = hsmName + hsmID + "." + ((stateName == null)?myCurStateName:stateName);
			...
			if (event.type ==CogEvent.EVTD_COG_PRIVATE_EVENT) {
				...
				switch (event.sig) {
					...
					/*	if(_initUseHistory){
							...
							tran(_initState, _initStateDoINIT);
		...
		*/
		protected function hsm_s_Active(event:CogEvent):void {
			...
			if (event.type ==CogEvent.EVTD_COG_PRIVATE_EVENT) {
				...
				//	trace("DISPATCH 111111111111" + mySource.call (this, TRACE_EVT));
					t = s.call(this,event);
					REQUIRE( t != myCurState, hsmName + hsmID +" statemachine, hit infinite state loop!, state(event) should return null or another state");
					s = t;
		...
		/*...........Root State, that underlies everythign......................................................*/
		 public function s_root(event:CogEvent):Function {
			...
			//trace (hsmName + hsmID + __cStateName +".RootState(" + e.toString () + ")");
			return null;
		}
	}
