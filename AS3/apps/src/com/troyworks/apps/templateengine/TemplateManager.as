package com.troyworks.apps.templateengine {
	...
	public class TemplateManager extends BaseStatefulObject{
		...
		//
		public function getLabel(label_str : String, strip : Boolean = true, lineBreak : Boolean = false) : String {
			if (label_str == null || label_str == "") {
					return "";
			}
			label_str = StringUtil.trim(label_str);
			var s_array:Array = label_str.split(ENDCHAR);
			var r_array:Array = new Array();
			var str:String;
			if (s_array.length>1) {
					for (var i:Number = 0; i<s_array.length; i++) {
						var t_array:Array = s_array[i].split(STARTCHAR);
					if (t_array.length>1) {
							r_array.push(t_array[0]);
						//trace("trying to get lable");
							str= getLabelFromDictionary(t_array[1], strip);
						if (lineBreak) {
				...
				// trace("getting label " + label_str + " found " + r_array.join(""));
					...
					str= getLabelFromDictionary(label_str, strip);
		...
		public function getLabelFromDictionary(pH : String, strip : Boolean) : String {
			...
			var lbl:String = null;
			for(var i:Number = 0; i < labelDict.length; i++){
				var dictionaryName:String = labelDict[i];
		...
		public function setLabelData(dictionaryName : String, dictionary : Object) : void {
			...
			var len:Number = labelDict.length;
			while(len--){
				var name:String = labelDict[len];
		...
		*/
		...
		public function replaceLabels(moddata:Object, useNameIfNotFound:Boolean, useDetailNameIfNotFound:Boolean) : Object {
			moddata.LABEL = getLabel(moddata.LABEL, useNameIfNotFound);
			var _array:Array = moddata.ITEMS as Array;
