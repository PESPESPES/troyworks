package com.troyworks.controls.tcarouselmenus { 
	...
	import com.troyworks.controls.tmediaplayer.ui.FramedMediaPlayer;
	...
	public class CarouselMenu extends BaseComponent {
		...
		public var PlaylistStart : Class;
		public static var SELECTED_SLIDE_EVT : Signals = Signals.getNextSignal("SELECTED_SLIDE");
		...
		public function parsePlayList(playList : RandomizedPlayList) : void {
			...
			while(playList.hasNext()) {
				...
				var line : Line1D = new Line1D("PlayListItem", 1, playlistViewModel.Z.position, itemWidth, null);
				line.data = page;
		...
		public function hide() : void {
			tran(s2_hiddenCarousel);
		}

		public function show() : void {
			tran(s2_animatingCarousel);
		}
		...
		override public function onLoad(init : Boolean = true) : void {
			...
			//XXX	_owidth = view.width;
			//playlistvisiblewindow_mc.visible =true;
			dispatchEvent(new Event(EVT_LOADED));//ASSETS_LOADED_EVT);
	//		
		}
		...
		public function onMouseMove() : void {
			var b2 : Bounds2 = new Bounds2(playlistvisiblewindow_mc.getBounds(view));
			...
			if(b2.contains(playlistvisiblewindow_mc.mouseX, playlistvisiblewindow_mc.mouseY)) {
				...
				if (playlistvisiblewindow_mc.mouseX < (50)) {
					...
					rate = -5;
					//(this.mouseX-(centerX-100 + _xadj));//_owidth;
					trace("GO -- " + rate);
					...
					trace("SPEED C " + speed);
					if (rate != 0) {
						if (lastMousePos == "LEFT_SIDE" && view.mouseX > centerX) {
							rate = 0;
						} else if (lastMousePos == "RIGHT_SIDE" && view.mouseX < centerX) {
							rate = 0;
						}
					}
				}
		...
		//////////////////////// LEVEL 0 STATES////////////////////////////
		/*.................................................................*/
		override function s1_creatingView(e : CogEvent) : Function {
			...
			switch (e.sig) {
				case SIG_ENTRY :
					...
					if(USEBOOKENDS){
						//////////// PLACE START BOOKEND /////////////////
						var menuStart : MovieClip = new PlaylistStart();//
						menuStart.x = cx;//attachMovie("PlaylistStart", "photoStart", getNextHighestDepth(),ini1);
						menuStart.width =40;
						view.addChild(menuStart);
					...
					for (var i : Number = 0; i < playlistViewModel.children.length; i++) {
						...
						menuItem.addEventListener("CLICK", this.createCallback(SELECTED_SLIDE_EVT));
						menuItem.addEventListener("CLICK",  this.onChildClicked);
	...
	//TODO CONVERT to AS3
		...
		////////////////////////////LEVEL 2 States ///////////////////////
		/*.................................................................*/
		function s2_animatingCarousel(e : CogEvent) : Function
		{
		//	this.onFunctionEnter ("s2_animatingCarousel-", e, []);
			switch (e.sig)
			{
				...
				//	trace("000000000000000000000000000" + Trace.me( parent.main_window)+ "000000000000000000000000000");
					trace("000000000000000000000000000000000000000000000000000000");
				//TODO CONVERT to AS3
						/*
		...
		function s2_pauseCarousel(e : CogEvent) : Function
		{
		...
		function s2_hiddenCarousel(e : CogEvent) : Function
		{
	...
	/*TODO convert to AS3		_mc.onRollOut = function() {
				trace("ROLLOVER");
